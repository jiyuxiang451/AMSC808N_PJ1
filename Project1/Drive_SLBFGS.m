...
%% dividing hyperplane: w'*x + b % in this problem one have quadratic surface
dim = nPCA + nPCA^2 + 1; 
% Y = (label*ones(1,dim + 1)).*[Xtrain,ones(size(Xtrain,1),1)]; %
% previously, Y*w is the test fucntion, now we no longer need this
%% to understand
% disp(Y( 1:3, :))
disp(size(Xtrain, 1))
disp(size(Xtrain, 2))
%% use Stochastic L-BFGS
lam = 0.001; % Tikhonov regularization parameter
func = @(I,Xtrain,label,w)qloss(I,Xtrain,label,w,lam);
gfun = @(I,Xtrain,label,w)qlossgrad(I,Xtrain,label,w,lam);
w = ones(dim,1); % initilization of the parameters
% params for Stochastic L-BFGS
frac = 25;
bsz = ceil(Ntrain/frac); % batch size
kmax = 51; %1e3*frac; % the max number of iterations
% tol = 1e-4; we set tol inside SLBFGS

% call the optimizer
[w,f,gnorm, toctime] = SLBFGS(func,gfun,Xtrain,label,w,bsz,bsz,kmax);
...
tmp = diag(Xtest*W*Xtest');
test = testlabel.*tmp + ((testlabel*ones(1,d)).*Xtest)*v + testlabel*b;
